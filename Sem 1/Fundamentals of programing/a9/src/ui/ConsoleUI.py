...
class ActivityPlannerUI:
    ...
    def display_main_menu(self):
        ...
        print(f"{self.UNDO_COMMAND}. Undo")
        print(f"{self.REDO_COMMAND}. Redo")
    ...
    def run(self):
        while True:
            ...
            if choice == self.MENU_MANAGE_PERSONS:
                ...
                break
            elif choice == self.UNDO_COMMAND:
                self.undo_redo_manager.undo(self.service)
            elif choice == self.REDO_COMMAND:
                self.undo_redo_manager.redo(self.service)
    ...
    def add_person(self):
        ...
        try:
            ...
            add_person_action = AddPerson(person_id, name, phone_number)
            self.undo_redo_manager.do(add_person_action, self.service)
    ...
    def remove_person(self):
        ...
        try:
            person_id = int(person_id)
            removed_person_action = RemovePerson(person_id)
            self.undo_redo_manager.do(removed_person_action, self.service)
    ...
    def update_person(self):
        ...
        try:
            person_id = int(person_id)
            update_person_action = UpdatePerson(person_id, name, phone_number)
            self.undo_redo_manager.do(update_person_action, self.service)
            print("Person updated successfully!")
        except Exception as Error:
            print("Services Error:\n" + str(Error))

    def list_persons(self):
        persons = self.service.persons_list
    ...
    def add_activity(self):
        ...
        persons_id = input("Enter Person IDs (comma-separated): ")
        if persons_id == '':
            persons_id = []
        else:
            persons_id = persons_id.split(',')
        ...
        try:
            ...
            add_activity_action = AddActivity(activity_id, persons_id, date, start_time, end_time, description)
            self.undo_redo_manager.do(add_activity_action, self.service)
    ...
    def remove_activity(self):
        ...
        try:
            activity_id = int(activity_id)
            remove_activity_action = RemoveActivity(activity_id)
            self.undo_redo_manager.do(remove_activity_action, self.service)
    ...
    def update_activity(self):
        ...
        persons_id = input("Enter New Person IDs (comma-separated): ")
        ...
        try:
            ...
            if persons_id=='':
                ...
                persons_id=persons_id.split(',')
            update_activity_action = UpdateActivity(activity_id, persons_id, date, start_time, end_time, description)
            self.undo_redo_manager.do(update_activity_action, self.service)
            print("Activity updated successfully!")
        except Exception as Error:
            print("Services Error:\n" + str(Error))

    def list_activities(self):
        activities = self.service.activities_list
    ...
    def busiest_days(self):
        print("Busiest Day:")
        busiest_days = self.service.busiest_days()
        for day in busiest_days:
            print(f"{day[0]}: {day[1]} minutes")
