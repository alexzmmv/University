...
class BinaryFileRepository(MemoryRepository):
    def __init__(self, persons_filename, activities_filename):
        super().__init__()
        self.__persons_filename = persons_filename
        self.__activities_filename = activities_filename
        self.__persons , self.__activities = self.__load_file()
    ...
    def __load_file(self):
        try:
            with open(self.__persons_filename, 'rb') as file:
                persons = pickle.load(file)
        except EOFError:
            persons = {}
        try:
            with open(self.__activities_filename, 'rb') as file:
                activities = pickle.load(file)
        except EOFError:
            activities = {}
        return persons, activities
    def __save_file(self):
        with open(self.__persons_filename, 'wb') as file:
            pickle.dump(self.__persons, file)
        with open(self.__activities_filename, 'wb') as file:
            pickle.dump(self.__activities, file)
    ...
    def update_activity(self, activity_id, persons_id, date, start_time, end_time, description, valid=False):
        if not valid:
            ...
            for activity_key in self.__activities.keys():
                if self.__activities[activity_key].is_time_conflict(date=date, start_time=start_time,
                                                                    end_time=end_time):
                    raise Repository_error("Time conflict!")
            for person_id in persons_id:
                if person_id not in self.__persons.keys():
    ...
    def get_activities_by_date_time(self, date):
        ...
        activities_list = []
        for activity_id in self.__activities.keys():
            if self.__activities[activity_id].date.date() == date:
