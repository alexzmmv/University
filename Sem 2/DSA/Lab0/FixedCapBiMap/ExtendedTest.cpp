...
void testremoveKey(){
    ...
    //remove key 1
    ValuePair t= m2.removeKey(1);
    //check if the values of key 1 were removed
    assert(t== TElem(1,2));
    //check if the values of key 1 are not in the map anymore
    assert(m2.search(1).first == NULL_TVALUE);
    assert(m2.search(1).second == NULL_TVALUE);
    //remove key 2
    t= m2.removeKey(4);
    //check if the values of key 2 were removed
    assert(t.first == NULL_TVALUE);
    t=m2.removeKey(3);
    //check if the values of key 3 were removed
    assert(t == TElem(6,NULL_TVALUE));
    int size=m2.size();
    //check if the size of the map is correct
    assert(size==3);
}
